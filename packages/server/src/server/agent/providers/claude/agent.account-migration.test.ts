...
async function fixture(
  ...
  const channels = [openQuery(), openQuery()];
  ...
  }).resumeSession({ provider: "claude", sessionId: sourceId, metadata: { cwd: root } });
  ...
  return { root, source, target, sourceId, session, channels, launches, events };
}

test("switches a live background agent and submits recovery using its saved native ID", async () => {
...
test.each([null, "", '{"type":"user","message":{"content":"ok"}}\n{broken'])(
...
test.each(["local_bash", "local_workflow", "unknown"])(
...
test("requests recovery on the original account if the target cannot be written", async () => {
  const f = await fixture();
  try {
...
test("recovers after confirmed process exit even when SDK return rejects", async () => {
  const f = await fixture();
...
test("includes late children and reports shell tasks created while shutting down", async () => {
  const f = await fixture();
  try {
...
test("reports a late shell even if the last agent completed during shutdown", async () => {
  const f = await fixture();
  try {
    f.channels[0]!.beforeClose(() => {
      f.channels[0]!.push({
...
test("a timed out old pump cannot complete the new recovery turn", async () => {
  const f = await fixture();
  try {
