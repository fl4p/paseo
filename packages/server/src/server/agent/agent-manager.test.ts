...
test("a rejected terminate leaves the stuck run in place so Stop can be retried", async () => {
  ...
  try {
    ...
    expect(client.resumeOverrides).toHaveLength(0);
    expect(manager.hasInFlightRun(agentId)).toBe(true);
    expect(manager.getAgent(agentId)?.lastError).toContain("could not confirm");
...
test("a kill that ended the turn but was not confirmed marks the agent errored and resumes nothing", async () => {
  // Terminating settles the turn (the dying provider rejects the prompt) and only then fails to
  // confirm the tree dead: Stop fails, the agent is errored, and no fresh session is started.
  ...
  try {
    ...
    expect(manager.getAgent(agentId)).toMatchObject({
      lifecycle: "error",
      lastError: expect.stringContaining("Reload the agent to retry"),
    });
    expect(session.closed).toBe(false);
    expect(client.resumeOverrides).toHaveLength(0);
  } finally {
