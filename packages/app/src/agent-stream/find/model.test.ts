...
describe("findTranscriptMatches", () => {
  ...
  it("is case-insensitive and literal", () => {
    expect(findTranscriptMatches({ items, query: "DEPLOY" })).toHaveLength(2);
    expect(findTranscriptMatches({ items, query: "w.dget" })).toEqual([]);
  });

  it("reports nothing for an empty query", () => {
    expect(findTranscriptMatches({ items, query: "" })).toEqual([]);
  });

  it("reports nothing for an empty transcript", () => {
    expect(findTranscriptMatches({ items: [], query: "widget" })).toEqual([]);
  });

  it("skips a row shorter than the query instead of scanning it", () => {
    expect(findTranscriptMatches({ items, query: "a much longer query than any row" })).toEqual([]);
  });
